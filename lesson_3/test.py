...
class WooFi(Base):
    async def swap_eth_to_token(
            ...
            amount: TokenAmount,
            token_name: str,
            token_contract: Contracts,
            slippage: float = 1
    ):
        failed_text = f'Failed swap ETH to {token_name} via WooFi'
        ...
        if token_name.upper() is 'USDT':
            token_price = 1
        else:
            token_price = await self.get_token_price(token=token_name.upper())

        min_to_amount = TokenAmount(
            amount=eth_price * float(amount.Ether) / token_price * (1 - slippage / 100),
        ...
        if receipt:
            return f'{amount.Ether} ETH was swapped to {min_to_amount.Ether} {token_name} via WooFi: {tx.hash.hex()}'

        return f'{failed_text}!'

    async def swap_token_to_eth(
            self,
            token_name: str,
            token_contract: Contracts,
            amount: Optional[TokenAmount] = None,
            slippage: float = 1):
        failed_text = f'Failed swap {token_name} to ETH via WooFi'
        contract = await self.client.contracts.get(contract_address=Contracts.ARBITRUM_WOOFI)
        ...
        eth_price = await self.get_token_price(token='ETH')
        if token_name.upper() == 'USDT':
            token_price = 1
        else:
            token_price = await self.get_token_price(token=token_name.upper())

        min_to_amount = TokenAmount(
            amount=float(amount.Ether) * token_price / eth_price * (1 - slippage / 100)
        )
        ...
        receipt = await tx.wait_for_receipt(client=self.client, timeout=200)
        if receipt:
            return f'{amount.Ether} {token_name} was swapped to {min_to_amount.Ether} ETH via WooFi: {tx.hash.hex()}'
